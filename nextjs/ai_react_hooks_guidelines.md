...
- **Bundle Awareness**: Любой hook должен соответствовать требованиям [`ai_bundle_optimize_nextjs_guideline.md`](./ai_bundle_optimize_nextjs_guideline.md) — нельзя тянуть тяжёлые библиотеки напрямую. Используйте динамические импорты или server actions, если hook выполняет HTTP/форматирование.
