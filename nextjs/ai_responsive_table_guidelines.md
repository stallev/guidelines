...
> ⚠️ **Bundle‑требование**: перед разработкой изучите [`ai_bundle_optimize_nextjs_guideline.md`](./ai_bundle_optimize_nextjs_guideline.md). Любая таблица должна по умолчанию быть серверным компонентом; интерактивные действия (модалки, меню) подключаем через небольшие client‑островки и динамические импорты. Никаких тяжелых библиотек (например, `react-table`) без явного обоснования и ленивой загрузки.
