...
- Выполняет все требования из [`ai_bundle_optimize_nextjs_guideline.md`](./ai_bundle_optimize_nextjs_guideline.md): утилиты не должны тянуть большие библиотеки в client bundle. Если требуется тяжёлый пакет (например, `date-fns`), используйте его только на сервере или через динамический импорт.
